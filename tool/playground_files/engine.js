...
(function (global, factory) {
    ...
    const { replace: StringReplace, toLowerCase: StringToLowerCase, indexOf: StringIndexOf, charCodeAt: StringCharCodeAt, slice: StringSlice, split: StringSplit$1, } = String.prototype;
    ...
    const { DOCUMENT_POSITION_CONTAINED_BY, DOCUMENT_POSITION_CONTAINS, } = Node;
    const { insertBefore, removeChild, appendChild, compareDocumentPosition, } = Node.prototype;
    /**
    ...
    function getOuterHTML(node) {
        switch (node.nodeType) {
            case Node.ELEMENT_NODE: {
                const { tagName, attributes: attrs } = node;
    ...
    */
    ...
    const { setAttribute: setAttribute$1, removeAttribute: removeAttribute$1, getAttribute: getAttribute$1 } = Element.prototype;
    const isNativeShadowRootAvailable = typeof window.ShadowRoot !== "undefined";
    const { replace: StringReplace$1, toLowerCase: StringToLowerCase$1, toUpperCase: StringToUpperCase, } = String.prototype;
    ...
    function patchCustomElementAttributeMethods(elm) {
        const { setAttribute: originalSetAttribute, removeAttribute: originalRemoveAttribute, } = elm;
    ...
    function parentElementDescriptorValue() {
        ...
        const ownerShadow = getShadowRoot(getNodeOwner(this));
    ...
    const EventPatchDescriptors = {
        ...
        target: {
            get() {
                ...
                // }
                const myCurrentShadowKey = (eventContext === EventListenerContext.SHADOW_ROOT_LISTENER) ? getNodeKey(currentTarget) : getNodeOwnerKey(currentTarget);
                // Determine Number 2:
                ...
                let nodeOwnerKey = getNodeOwnerKey(closestTarget);
                while (nodeOwnerKey !== myCurrentShadowKey && nodeOwnerKey !== myOwnerKey) {
                    ...
                    nodeOwnerKey = getNodeOwnerKey(closestTarget);
                }
                /**
    ...
    const { setAttribute: setAttribute$2, getAttribute: getAttribute$2, setAttributeNS: setAttributeNS$1, getAttributeNS: getAttributeNS$1, removeAttribute: removeAttribute$2, removeAttributeNS: removeAttributeNS$1, addEventListener: addEventListener$1, removeEventListener: removeEventListener$1, } = Element.prototype;
    const parentNodeGetter$1 = getOwnPropertyDescriptor(Node.prototype, 'parentNode').get;
    const parentElementGetter$1 = hasOwnProperty.call(Node.prototype, 'parentElement') ?
        getOwnPropertyDescriptor(Node.prototype, 'parentElement').get :
        getOwnPropertyDescriptor(HTMLElement.prototype, 'parentElement').get; // IE11
    ...
    // the proxy is a compat proxy, and define the unwrap method accordingly.
    // @ts-ignore
    const { getKey: getKey$1 } = Proxy;
    const unwrap$1 = getKey$1 ?
        (replicaOrAny) => (replicaOrAny && getKey$1(replicaOrAny, TargetSlot$1)) || replicaOrAny
        : (replicaOrAny) => (replicaOrAny && replicaOrAny[TargetSlot$1]) || replicaOrAny;
    ...
    const { addEventListener: addEventListener$2, removeEventListener: removeEventListener$2 } = Element.prototype;
    ...
    function apply$1() {
        HTMLElement.prototype.click = function () {
            addEventListener$2.call(this, 'click', handleClick);
            try {
                ...
                removeEventListener$2.call(this, 'click', handleClick);
            }
        };
    }
    ...
    */
    ...
    const { removeChild: removeChild$2, appendChild: appendChild$2, insertBefore: insertBefore$2, replaceChild } = Node.prototype;
    ...
    function buildCustomElementConstructor(Ctor, options) {
        ...
        if (isObject(options) && !isNull(options)) {
            ...
            if (fallback === true) {
                normalizedOptions.fallback = true;
            }
        }
        ...
        LightningWrapperElement.observedAttributes = getOwnPropertyNames(props);
    ...
    exports.Element = LightningElement;
