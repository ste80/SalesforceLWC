...
(function (global, factory) {
	...
	(factory((global.Locker = global.Locker || {})));
  ...
  if (isArray(st)) {
    ...
    if (secureToRaw.get(st)) {
      ...
      for (let n = 0; n < st.length; n++) {
        ...
        ref[insertIndex++] = unwrap(from, st[n]);
      }
    } else {
      ref = [];
    }
...
function SecureElement(el, key) {
  ...
  if (!prototypeInfo) {
    ...
    const prototypicalInstance = create(prototype);
  ...
  o = create(prototypeInfo.prototype);
...
function SecureNodeList(raw, toKey, options) {
  const shadowTarget = create(getPrototypeOf(raw));
...
let deepUnfilteringTypeHook;

let isUnfilteredTypeHook;
...
function filterObject(key, raw, options) {
  ...
  if (raw === window) {
    ...
    return getEnv(key).document;
  } else if (raw === window.location) {
    return getEnv(key).location;
  }
  ...
  if (isArray(raw)) {
    ...
    if (filterTypeHook) {
      swallowed = filterTypeHook(raw, key, belongsToLocker);
    }
    if (swallowed) {
...
function createFilteringProxy(raw, key) {
  ...
  const surrogate = create(getPrototypeOf(raw));
...
function isUnfilteredType(raw, key) {
  ...
  if (isUnfilteredTypeHook && isUnfilteredTypeHook(raw, key)) {
    return true;
  }
...
function deepUnfilterMethodArguments(st, baseObject, members) {
  ...
  for (const property in members) {
    ...
    if (deepUnfilteringTypeHook) {
      rawValue = deepUnfilteringTypeHook(fromKey, value);
    }
...
function deepUnfilter(key, baseObject, members) {
  ...
  for (const property in members) {
    value = members[property];
    if (deepUnfilteringTypeHook) {
      rawValue = deepUnfilteringTypeHook(key, value);
    }
    // If the hooks failed to unfilter the value, do the deep copy, unwrapping one item at a time
    if (rawValue === value) {
      if (isArray(value)) {
...
function SecureURL(raw) {
  const SecureURLMethods = create(null, {
    createObjectURL: {
...
function SecureCustomEventFactory(CustomEventCtor, key) {
  ...
  SecureCustomEvent = function(...args) {
    ...
    const filteredArguments = deepUnfilterMethodArguments(getEnv(key), [], args);
...
function SecureWindow(sandbox, key) {
  ...
  const emptyProto = create(null, props);
...
function getBase(key) {
  return keyToBase.get(key);
}

function setBase(key, url) {
